...
# src: https://github.com/iamntz/sublime-per-project-builds
...
#             "autorun_when_single": true,
#             "shell_cmd": "make -makefile=MAKEFILE2",
...
class ProjectBuildCommand(sublime_plugin.WindowCommand):
    def run(self):
        win = self.window
        project = win.project_data()

        self.build_systems = project.get("build_systems") # Get all sections
        self.config = self.build_systems # Get the config section and remove it from list
        self.timer = None
